    ...
    <div class="max-w-6xl mx-auto w-full space-y-6">
      ...
      <div class="grid grid-cols-2 gap-8">
        <div>
          ...
          />
          <p v-if="errors.workType" class="text-red-500 text-sm mt-1">
            {{ errors.workType }}
          </p>
        </div>
...
export default {
  ...
  methods: {
    ...
    previewIDImage(e) {
      ...
      if (file) {
        if (file.size > 5 * 1024 * 1024) {
          this.errors.idImage = "ID image must be less than 5MB";
          return;
        }
    ...
    validateForm() {
      ...
      if (!this.form.workType.trim()) {
        e.workType = "Work type is required";
        valid = false;
      }
      ...
      if (!this.form.idImage) {
        e.idImage = "ID image is required";
        valid = false;
      }
